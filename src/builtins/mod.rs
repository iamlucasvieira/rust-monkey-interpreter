...
fn builtin_len(args: Vec<Rc<Object>>) -> Result<Rc<Object>> {
    if args.len() != 1 {
        return Err(Error::TypeMismatch(format!("wrong number of arguments. got={}, want=1", args.len())).into());
    }

    match &*args[0] {
        Object::String(s) => Ok(Rc::new(Object::Integer(s.len() as i64))),
        _ => Err(Error::TypeMismatch(format!("argument to `len` not supported, got {}", args[0].object_type())).into()),
    }
}
